...
cnx = st.experimental_connection('mpg_pmsys', type='sql')
...
def gps_statistics():
   ...
   # # Connection to DB
   dataset = cnx.query('select * from gps')
   df_team_A = cnx.query("SELECT * FROM gps WHERE player_name LIKE 'TeamA%' ")
   df_team_B = cnx.query("SELECT * FROM gps WHERE player_name LIKE 'TeamB%' ")
   Players_list = cnx.query("SELECT * FROM gps ORDER BY date, time ASC")
   ...
   # # Concat
   team = dataset #pd.concat([df, df0, df1, df2, df3, df4, df5, df6], axis=1)
   ...
   with tab2: # --- MAP VISUALIZATION ---
      ...
      # import base64
      # image_filename = 'D:/Simula/Git 2023/soccer-dashboard/data/gps/Football.png' #'Football.png' in cloud
...
# Plot
      ...
      def create_plot(plot_type, data, x_col, y_col, color):
          if plot_type == 'Bar plot':
              return px.bar(data, x=x_col, y=y_col, color_discrete_sequence=[color])
          elif plot_type == 'Line plot':
              return px.line(data, x=x_col, y=y_col, color_discrete_sequence=[color])
          elif plot_type == 'Scatter plot':
              return px.scatter(data, x=x_col, y=y_col, color_discrete_sequence=[color])

      # Plot_1
      st.divider()
      st.header("Heart rate over Time")
      data_subset = dataset.iloc[0:slider_analyse_1]
      fig1 = create_plot(genre, data_subset, 'time', 'heart_rate', color)
      st.plotly_chart(fig1, use_container_width=True)

      # Plot_2
      st.divider()
      st.header("Acceleration over Time")
      fig2 = create_plot(genre, data_subset, 'time', 'accl_x', color)
      st.plotly_chart(fig2, use_container_width=True)

      # Plot_3
      st.divider()
      st.header("Rotation over Time")
      fig3 = create_plot(genre, data_subset, 'time', 'gyro_x', color)
   ...
   with tab5: #--- Player GPS Report --- IT HAS TO BE AN INDEPENDANT PAGE FROM SIDEBAR : ANOTHER PY.FILE
      ...
      st.write("")
      st.write("Select a player to be compared:")
      ...
      with col1:
         ...
         # Box 1 left
         st.date_input("Select date 1",datetime.date(2022, 1, 1), key= "date1")  

         # Box 2 left 
         Session = ["Select a session",'Session_A', 'Session_B']
         selected_players = st.selectbox("Session 1",Session, key="Session")    
         ...
         multi1 = st.multiselect('Select columns :',['time','heart_rate','speed','inst_acc_impulse', 'distance'],['time', 'heart_rate'], key='A')
         ...
         fig.add_trace(go.Bar(y=[2, 3, 1], name='Trace 1'), row=1, col=1)
         ...
         fig.update_layout(height=700, showlegend=False, barmode='group')
         st.plotly_chart(fig, use_container_width=True)
         st.divider()
      ...
      with col2:
         ...
         # Box 1 right
         st.date_input("Select date 2",datetime.date(2022, 1, 1), key= "date2")  

         # Box 2 right 
         Session = ["Select a Session",'Session_A', 'Session_B']
         selected_players = st.selectbox("Session 2",Session, key="Session1") 

         # Box 3 right
         st.write("")

         multi2 = st.multiselect('Select columns :',['time','heart_rate','speed','inst_acc_impulse', 'distance'],['time', 'heart_rate'], key='B')
         #,'accl_x','accl_y', 'accl_z', 'gyro_x', 'gyro_y', 'gyro_z','hacc', 'hdop','signal_quality','num_satellites',
